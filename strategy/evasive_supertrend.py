"""
Evasive SuperTrend breakout strategy.
...
from indicators.evasive_supertrend import EvasiveSuperTrend, SIGNAL_UPTREND, SIGNAL_DOWNTREND
...
class EvasiveSuperTrendStrategy(BaseStrategy):
    """Evasive SuperTrend strategy — adaptive single band for entry and exit."""
    ...
    def __init__(self, params: Dict[str, Any] = None):
        ...
        self.noise_threshold = self.params.get("noise_threshold", 1.0)
        self.expansion_alpha = self.params.get("expansion_alpha", 0.5)

        self.evasive_st = EvasiveSuperTrend(
            atr_length=int(self.atr_length),
            multiplier=float(self.multiplier),
            noise_threshold=float(self.noise_threshold),
            expansion_alpha=float(self.expansion_alpha),
            name="EvasiveST"
        )
    ...
    def generate_backtest_signals(self, df: pd.DataFrame) -> Dict[str, np.ndarray]:
        ...
        st_series = self.evasive_st.calculate(candles, return_series=True)
        ...
        if not st_series:
            return {
                ...
                "meta_is_noisy": np.zeros(n)
            }
            
        signal = st_series["signal"]
        st_val = st_series["supertrend"]
        is_noisy = st_series["is_noisy"]
        ...
        return {
            ...
            "meta_is_noisy": is_noisy.astype(float)
        }
    ...
    async def calculate_indicators(self, client: DeltaExchangeClient, symbol: str, timeframe: str, skip_boundary_check: bool = False, force_recalc: bool = False, historical_candles: Optional[List[Dict[str, Any]]] = None) -> Optional[Dict[str, Any]]:
        try:
            ...
            st_result = self.evasive_st.calculate(candles)
            if not st_result:
                return None
    ...
    def generate_entry_signal(self, setup_id: str, previous_state: Optional[Dict[str, Any]], indicators_data: Dict[str, Any]) -> Optional[EntrySignal]:
        try:
            st = indicators_data.get("evasive_st")
            ...
            if entry_side == "long":
                ...
                if current_price >= trigger_price:
                    return EntrySignal(
                        ...
                        immediate=True, reason='EvasiveST flip to uptrend (immediate)'
                    )
            else:
                ...
                if current_price <= trigger_price:
                    return EntrySignal(
                        ...
                        immediate=True, reason='EvasiveST flip to downtrend (immediate)'
                    )

            return EntrySignal(
                side=entry_side, trigger_price=trigger_price, stop_loss=st_value,
                immediate=False, reason=f"EvasiveST flip to {'uptrend' if entry_side == 'long' else 'downtrend'}"
            )
    ...
    def generate_exit_signal(self, setup_id: str, position_side: str, indicators_data: Dict[str, Any]) -> Optional[ExitSignal]:
        try:
            ...
            if should_exit:
                return ExitSignal(
                    reason=f"EvasiveST flip to {'uptrend' if current_signal == 1 else 'downtrend'}",
    ...
    def get_cache_mapping(self, indicators_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "display_details": {
                "Evasive ST": f"{emoji} {signal_text} @ ${st_val}",
                "ATR": atr,
                "Noise": noise_str,
            }
        }
